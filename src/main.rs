...
async fn main() {
    ...
    match &cli.command {
        ...
        Commands::ReceivePayment { amount_sats, description } => {
            let sdk = connect().await;
            ...
            info!("Invoice: {}, expected opening fee (msat): {:?}", invoice.ln_invoice.bolt11, invoice.opening_fee_msat);
        ...
        Commands::ChatGpt { prompt } => {
            ...
            if resp.status() == 402 {
                ...
                info!("Paying lightning invoice to get access to the API: {}", invoice);
                let payresult = sdk
                    .send_payment(invoice, None)
                    .await
                    .unwrap();
                ...
                info!("Calling http 402 api again, now with header Authorization {}", header);
                resp = client
                    .post(url)
...
async fn connect() -> Arc<BreezServices> {
    let breez_sdk_api_key = get_env_var("BREEZ_API_KEY")
        .expect("set the 'BREEZ_API_KEY' environment variable");
    let greenlight_invite_code = get_env_var("GREENLIGHT_INVITE_CODE")
        .expect("set the 'GREENLIGHT_INVITE_CODE' environment variable");
    let phrase = get_env_var("MNEMONIC")
        .expect("set the 'MNEMONIC' environment variable");
